...
class apiAuthenticationsController extends Controller
{
    ...
    public function registration(Request $request)
    {
        ...
        try {
            ...
            $api_url = env('API_BASE_URL').'/customer-signup';
            ...
            $api_url = env('API_BASE_URL').'/customer-login?password='.$reg_password.'&username='.$reg_email;
    ...
    public function initLogin(Request $request)
    {
        ...
        try {
            $api_url = env('API_BASE_URL').'/customer-login?password='.urlencode($login_password).'&username='.urlencode($login_email);
            $curlOutput = HandleApi::getCURLOutput($api_url, 'POST', []);
            $decodedData = json_decode($curlOutput, true);
